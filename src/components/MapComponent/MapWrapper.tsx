...
const Legend = () => {
  const map = useMap();
  const categories = [
    { color: "5cb85c", label: "More than 5km away" },
    { color: "d9534f", label: "Within 5km" },
  ];
  useEffect(() => {
    if (!map) return;
...
const MapWrapper = () => {
  ...
  function haversineDistance(point1: ILocation, point2: ILocation) {
    ...
    <div className="w-[50%] h-[70%]">
      {data ? (
        ...
        >
          ...
          />
          {data?.map((location: ILocation, index: number, arr: ILocation[]) =>
            arr
              ?.filter((_, innerIndex) => innerIndex !== index)
              .some((item) => haversineDistance(location, item) < 5) ? (
              <Circle
                key={index}
        ...
        <h1 className="text-center text-xl font-semibold text-blue-600/100 dark:text-blue-500/100">
          Nothing Found!
        </h1>
      )}
    </div>
